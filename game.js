...
const LEVELS = [
  ...
  // L5 Raid — boss arena: the minotaur orbits, flanking guards pace.
  // ANTI-LOOP RULES (the old L5 trapped the ball in an inescapable lower-centre
  // bounce loop): ① keep content above the drain funnel — nothing near the shared
  // outlane pegs at world (±3.05,-0.3). ② NO kicking-pop cluster in the lower centre:
  // two pops + a centre pin formed a pop→pin→pop triangle that re-energised the ball
  // each bounce while the funnel knocked it back up → a closed limit cycle. So: drop
  // the centre pin, and STAGGER the two pops to different heights (no symmetric L↔R
  // ping), with open drain below. Verified loop-free by /tmp/mb_final.mjs.
  { name: 'Raid', pal: { fog: 0x281a0a, hemiSky: 0xe0b87b, hemiGround: 0x33260f, key: 0xfff0d0, floor: 0x3a2e14, inlay: 0xffa320 },
    ...
    obs: [ [-2.4, 3.0, 0xffa320], [2.4, 3.0, 0xffa320] ],
    pops: [ [-1.3, 5.0, 0x2fd0ff], [1.3, 4.0, 0xffd23f] ] },
];
